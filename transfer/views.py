...
def upload_handle(request):
    ...
    # 判断是否为图片类型
    if os.path.splitext(pic.name)[-1].lower() not in [".jpg", ".png", ".bmp", ".tiff"]:
        raise ValidationError("TypeError: type must be jpg, png, bmp or tiff")

    # 重构
    image = load_image(os.path.join(input_dir, pic.name), False)
    ...
    # 将图片保存至output目录
    out.save(os.path.join(output_dir, pic.name))

    # 在数据中保存转换记录
    ProcessedPic.objects.create(pro_pic='transfer/output/%s' % pic.name)

    print(f"image saved: {pic.name}")

    # pic_transfer(args)
    return render(request, 'transfer/download_pic.html', {"pic": pic.name})
